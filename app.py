...
def render_content(tab):
    ...
    if tab == 'tab_1_graph':
        ...
        bad_locations = []  # Due to poor address location the API might send us points outside Bordeaux, i will identify them
                            # here and remove them from the dataset
        for _index, _loc in complete_df["geolocalisation"].iteritems():
            if type(_loc) != list:
                _points_as_list = [float(i) for i in _loc[1:-1].split(', ')]
            else:
                _points_as_list = _loc
            if 44.78 < float(_points_as_list[0]) > 44.92 or  -0.59 < float(_points_as_list[1]) > -0.4:
                bad_locations.append(_index)

        complete_df.drop(bad_locations, axis=0, inplace=True)
        for _libelle, _localisation, _coordinates, _debut, _fin, _address in zip(complete_df['libelle_split'], complete_df['localisation_split'], complete_df['geolocalisation'], complete_df['fields.date_debut'].dt.strftime('%d/%m'), complete_df['fields.date_fin'].dt.strftime('%d/%m'), complete_df["address"]):
            ...
            if type(_localisation) != list:
                _localisation = _localisation[2:-2].split(',')
